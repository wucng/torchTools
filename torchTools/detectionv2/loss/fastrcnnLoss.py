...
class RPNLoss(nn.Module):
    def __init__(self,cfg,device="cpu"):
        ...
        self.num_classes = cfg["network"]["RPN"]["num_classes"] # 只做背景与目标分类
        ...
        self.threshold_cls = cfg["network"]["proposal"]["threshold_cls"]
        self.nms_thres = cfg["network"]["proposal"]["nms_thres"]
        self.num_anchor = cfg["network"]["proposal"]["num_anchor"]
    ...
    def normalize(self, targets):
        ...
        for target in targets:
            ...
            gt_labels = torch.ones_like(target["labels"]) # 只做背景与前景分类
    ...
    def proposal(self, preds_list):
        with torch.no_grad():
            pred_clses, pred_boxes = preds_list
            pred_boxes = pred_boxes.detach()
            pred_clses = torch.sigmoid(pred_clses.detach())
            result = []
            ...
            for i, (pred_cls, pred_box) in enumerate(zip(pred_clses, pred_boxes)):
                ...
                # nms 过滤
                keep = nms2(pred_box, scores, self.nms_thres)
                pred_box, scores, labels = pred_box[keep], scores[keep], labels[keep]
                # 再按分数排序取前2000个
                keep = scores.sort(descending=True)[1][:self.num_anchor]
                pred_box, scores, labels = pred_box[keep], scores[keep], labels[keep]
                result.append({"boxes": pred_box, "scores": scores, "labels": labels})
                # result.append(pred_box)

        return result #torch.stack(result,0)
    ...
    def reverse_normalize(self, boxes):
        ...
        # 恢复到输入图像尺寸上
        resize = self.resize
        boxes = boxes * torch.as_tensor([resize[1], resize[0], resize[1], resize[0]],device=self.device,dtype=torch.float32).unsqueeze(0)
...
class RCNNLoss(nn.Module):
    def __init__(self,cfg,device="cpu"):
        ...
        # self.num_anchors = cfg["network"]["RCNN"]["num_boxes"]
        self.num_classes = cfg["network"]["RCNN"]["num_classes"]  # 只做背景与目标分类
        self.threshold_conf = cfg["work"]["loss"]["threshold_conf"]
        self.threshold_cls = cfg["work"]["loss"]["threshold_cls"]
        self.conf_thres = cfg["work"]["loss"]["conf_thres"]
        self.nms_thres = cfg["work"]["loss"]["nms_thres"]
        ...
        self.variance = cfg["network"]["prioriBox"]["variance"]
        self.clip = cfg["network"]["prioriBox"]["clip"]
        self.thred_iou = cfg["network"]["RCNN"]["thred_iou"]

        self.neg_pos_ratio = 3  # 正负样本比例1:3

    def forward(self,preds,proposals,targets):
        if "boxes" not in targets[0]:
            results = self.predict(preds,proposals, targets)
            results = [self.apply_nms(result) for result in results]
            return results
        else:
            return self.compute_loss(preds,proposals,targets)

    def compute_loss(self, preds_list, proposals,targets_origin):
        last_result = self.normalize(proposals,targets_origin)
        losses = {
            "loss_box_rcnn": 0,
            "loss_clf_rcnn": 0  # ,
            # "loss_iou":0
        ...
        bs = len(targets_origin)
        for i in range(bs):
            confidence = preds_list[i][0]
            predicted_locations = preds_list[i][1]
            gt_locations, labels = last_result[i]
            with torch.no_grad():
                loss = -F.log_softmax(confidence, dim=1)[:, 0]  # 计算背景置信度loss，后续需按loss从大到小排序，选择更新的mask
                mask = hard_negative_mining(loss, labels, self.neg_pos_ratio)
            ...
            losses["loss_box_rcnn"] += smooth_l1_loss * 5.
            losses["loss_clf_rcnn"] += classification_loss
        return losses

    def normalize(self, proposals,targets):
        last_result = []
        for i,target in enumerate(targets):
            ...
            locations = convert_boxes_to_locations(boxes, x1y1x2y22xywh(priorBoxXYXY), self.variance[0], self.variance[1])  # [8732,4]

            last_result.append([locations, labels])
        return last_result

    def predict(self, preds_list,proposals, targets_origin):
        """
        :param preds_list:
                   #（2个特征为例,batch=2）
        ...
        for i, (pred_cls, pred_box) in enumerate(preds_list):
            pred_cls = torch.sigmoid(pred_cls)
            pred_box = self.reverse_normalize(proposals[i],pred_box)
            # scores, labels = torch.softmax(pred_cls, -1).max(dim=1)
            scores, labels = pred_cls.max(dim=1)
            keep = (scores > self.threshold_cls) * (labels > 0)  # labels>0 跳过背景
            labels = labels - 1  # label从0 开始与yolo对应起来 排除掉背景
    ...
    def reverse_normalize(self,proposal, boxes):
        h, w = self.resize
        # [x0,y0,w,h]-->normalize 0~1--->[x1,y1,x2,y2]
        priorBoxXYXY = proposal
        boxes = convert_locations_to_boxes(boxes, x1y1x2y22xywh(priorBoxXYXY), self.variance[0], self.variance[1])
        boxes = xywh2x1y1x2y2(boxes)
        # 恢复到输入图像尺寸上
        boxes = boxes * torch.as_tensor([w, h, w, h], dtype=torch.float32,device=self.device).unsqueeze(0)
